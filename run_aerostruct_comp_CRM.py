...
if __name__ == "__main__":
    ...
    # Specify a tolerance for the coupled aerostructural solver
    solver_atol = 5e-6
    ...
    prob_dict = {'type' : 'aerostruct',
                 ...
                 'Re' : 0.348*295*.85*1./(1.45*1e-5), # cruise Reynolds number for characteristic Re length
                 'g' : 9.80665,            # [m/s^2] acceleration due to gravity
 
                 # Aircraft properties
                 'CT' : 0.53/3600,         # [1/s] specific fuel consumption
                 'R' : 14.307e6,           # [m] Range
                 'cg' : np.zeros((3)),     # center of gravity for the entire aircraft. Used in trim and stability calculations.
                 ...
                 'W_wing_factor' : 1.25,   # factor to account for extra wing weight
                 'beta' : 1.,              # weighting factor for mixed objective (see functionals.py)
                 'g_factor' : g_factor,    # load factor for maneuver cases
                 'rho_maneuver' : 0.922,        # density that gives the required dynamic pressure for the maneuver case using the cruise speed
                 }
    ...
    surf_dict = {'num_y' : 51,             # number of spanwise nodes for complete wing
                 'num_x' : 3,              # number f chordwise nodes
                 'exact_failure_constraint' : False, # Exact or KS failure constraint
                 'wing_type' : 'CRM',      # using OAS's CRM wing
                 'span_cos_spacing' : 0,   # uniform spacing
                 'chord_cos_spacing' : 0,  # uniform spacing
                 'CL0' : 0.0,              # additional CL
                 'CD0' : 0.0089,           # additional CD (for fuse, tails, nacelles, etc here)
                 'symmetry' : True,
                 'S_ref_type' : 'wetted',
                 'twist_cp' : np.array([4., 4., 4., 4., 4., 4.]), # [deg] inital values for twist distribution 
                 # The following are two thickness variables that differ from the single thickness variable in the standard OAS.
                 'skinthickness_cp' : np.array([0.01, 0.02, 0.03, 0.03, 0.03, 0.2]), # [m]
                 'sparthickness_cp' : np.array([0.01, 0.01, 0.01, 0.01, 0.01, 0.2]), #[m]
                 # Airfoil properties for viscous drag calculation
                 'k_lam' : 0.05,            # percentage of chord with laminar flow, used for viscous drag
                 't_over_c' : 0.12,         # original thickness to chord ratio for the airfoil used
                 'toverc_cp' : np.array([0.12, 0.12, 0.12, 0.12, 0.12, 0.12]), # initial values for streamwise thickness to chord cp distribution
    ...
    # OAS_prob.add_constraint('coupled.wing.S_ref', upper=414)
